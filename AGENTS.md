...
- Run `pnpm check:showroom-acceptance` for code changes that affect the showroom or F1 handoff flow, including `src/App.tsx`, `src/components/WelcomePage.tsx`, `src/components/ParticleBackground.tsx`, files under `src/components/showroom/**`, files under `src/lib/showroom-*.ts`, or any change to ignition, skip, enter-app handoff, overlay visibility, scroll lock, or keyboard/pointer showroom controls. Treat `output/playwright/showroom-acceptance-summary.json` or the GitHub Actions artifact as the reviewable evidence.
